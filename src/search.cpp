...
namespace {
  ...
  int futility_move_count(bool improving, Depth depth, const Position& pos) {
    return improving ? (3 + depth * depth * (1 + pos.walling()) + 2 * pos.blast_on_capture()) / (1 + pos.blast_on_capture())
                     : (3 + depth * depth * (1 + pos.walling()) + 2 * pos.blast_on_capture()) / (2 + pos.blast_on_capture());
...
#else
              delta = Value(10 * (1 + rootPos.captures_to_hand())) + int(prev) * prev / 15620;
#endif
          ...
          && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
      {
...
#else
              if (!pos.see_ge(move, Value(-222 - 120 * pos.captures_to_hand()) * depth)) // (~25 Elo)
#endif
                  continue;
          }
          else
          {
              ...
              if (   !ss->inCheck
                  ...
                  && ss->staticEval + (106 + 145 * lmrDepth + history / 52) * (1 + pos.check_counting()) <= alpha)
#endif
                  continue;
