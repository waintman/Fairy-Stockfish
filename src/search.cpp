...
#ifdef FAIRY_STOCKFISH
        if (uci_elo > 1346.6) {
            level = std::clamp(std::pow((uci_elo - 1346.6) / 143.4, 1 / 0.806), -20.0, 20.0);
        }
        else if (uci_elo) {
            level = std::clamp((uci_elo - 1346.6) / 143.4 + std::pow((uci_elo - 1346.6) / 500, 5), -20.0, 20.0);
        }
#endif
        else
...
#else
              delta = Value(16 * (1 + rootPos.captures_to_hand())) + int(prev) * prev / 19178;
#endif
...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (   !PvNode
        ...
        && depth <= 7
#else
        &&  depth <= 7 - 3 * pos.blast_on_capture()
#endif
...
#else
        Depth R = (1090 - 300 * pos.must_capture() - 250 * !pos.checking_permitted() + 81 * depth) / 256 + std::min(int(eval - beta) / 147, pos.must_capture() || pos.blast_on_capture() ? 0 : 5);
#endif
          ...
          && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
      {
...
#else
              if (!pos.see_ge(move, Value(-203 - 120 * pos.captures_to_hand()) * depth)) // (~25 Elo)
#endif
                  continue;
          }
          else
          {
              ...
              if (   !ss->inCheck
                  ...
                  && ss->staticEval + (122 + 138 * lmrDepth + history / 60) * (1 + pos.check_counting()) <= alpha)
#endif
                  continue;
