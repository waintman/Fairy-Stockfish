...
#else
        MovePicker mp(pos, ttMove, probCutBeta - ss->staticEval, &thisThread->gateHistory, &thisThread->captureHistory);
#endif
        ...
        while ((move = mp.next_move()) != Move::none())
            if (move != excludedMove && pos.legal(move))
            {
                ...
                ss->continuationHistory =
                  &this
                     ->continuationHistory[ss->inCheck][true]
#ifndef FAIRY_STOCKFISH
...
Value Search::Worker::qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
    ...
    while ((move = mp.next_move()) != Move::none())
    {
        ...
        ss->continuationHistory =
          &thisThread
             ->continuationHistory[ss->inCheck][capture]
#ifndef FAIRY_STOCKFISH
                                  [pos.moved_piece(move)]
#else
                                  [history_slot(pos.moved_piece(move))]
