...
namespace {
  ...
  struct Skill {
    Skill(int skill_level, int uci_elo) {
        ...
        if (uci_elo > 1320) {
            double e = double(uci_elo - 1320) / (3190 - 1320);
            level = std::clamp((((37.2473 * e - 40.8525) * e + 22.2943) * e - 0.311438), -20.0, 19.0);
        }
        else if (uci_elo) {
            double e = double(uci_elo - 1320) / (3190 - 1320);
            level = std::clamp((((37.2473 * e - 40.8525) * e + 22.2943) * e - 0.311438) + std::pow((uci_elo - 1346.6) / 500, 5), -20.0, 20.0);
        }
#endif
        else
...
#else
          delta = Value(10 * (1 + rootPos.captures_to_hand())) + int(prev) * prev / 15799;
#endif
...
#else
              if (!pos.see_ge(move, occupied, Value(-205 - 120 * pos.captures_to_hand()) * depth)) // (~25 Elo)
#endif
              {
                 if (depth < 2 - capture)
                    continue;
