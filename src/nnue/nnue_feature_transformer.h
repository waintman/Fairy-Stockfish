...
#else
        read_leb_128<WeightType>(stream, weights , HalfDimensions * FeatureSet::get_dimensions());
        read_leb_128<PSQTWeightType>(stream, psqtWeights, PSQTBuckets * FeatureSet::get_dimensions());
#endif
...
#else
        write_leb_128<WeightType>(stream, weights, HalfDimensions * FeatureSet::get_dimensions());
        write_leb_128<PSQTWeightType>(stream, psqtWeights, PSQTBuckets * FeatureSet::get_dimensions());
#endif
            ...
            for (; i >= 0; --i)
            {
                ...
                for (; st2 != end_state; st2 = st2->previous)
                    ...
                    FeatureSet::append_changed_indices<Perspective>(ksq, st2->dirtyPiece,
                                                                    removed[i], added[i]);
#else
                    FeatureSet::append_changed_indices(ksq, st2->dirtyPiece, Perspective,
                                                       removed[i], added[i], pos);
#endif
            }
        }
